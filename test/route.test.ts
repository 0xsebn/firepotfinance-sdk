import { Token, WETH, ChainId, Pair, TokenAmount, Route, AMBER } from '../src'
...
describe('Route', () => {
  ...
  it('supports ether input', () => {
    const route = new Route([pair_0_weth], AMBER)
    expect(route.pairs).toEqual([pair_0_weth])
    expect(route.input).toEqual(AMBER)
    expect(route.output).toEqual(token0)
  })

  it('supports ether output', () => {
    const route = new Route([pair_0_weth], token0, AMBER)
    expect(route.pairs).toEqual([pair_0_weth])
    expect(route.input).toEqual(token0)
    expect(route.output).toEqual(AMBER)
  })
})
